...
import com.example.holybible.core.*
...
class BooksAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = when (viewType) {
        0 -> BooksViewHolder.Base(R.layout.test_layout.makeView(parent), bookListener)
    ...
    abstract class BooksViewHolder(view: View) : BaseViewHolder<BookUi>(view) {
        ...
        class Testament(view: View, private val collapse: CollapseListener) : Info(view) {
            private val collapseView = itemView.findViewById<CollapseView>(R.id.collapseImageView)
            override fun bind(item: BookUi) {
                super.bind(item)
